%% Load images.
image_dir = fullfile('/home/amanda/EECE-5698/lab4');
street_scene = imageDatastore(image_dir);
% Display images to be stitched
montage(street_scene.Files)
...
I = readimage(street_scene, 1);

% Initialize features for I(1)
grayImage = rgb2gray(I);
% grayImage = convolve2(grayImage,200);
% points = detectSURFFeatures(grayImage);
[y,x,~] = harris(grayImage,1000,'tile',[3 3],'disp');
...
numImages = numel(street_scene.Files);
...
for n = 2:numImages
    ...
    % Read I(n).
    I = readimage(street_scene, n);
    
    % Detect and extract SURF features for I(n).
    grayImage = rgb2gray(I);
%     grayImage = convolve2(grayImage,200);
    % points = detectSURFFeatures(grayImage);
    [y,x,~] = harris(grayImage,800,'tile',[3 3],'disp');
    points = cornerPoints([x y]);
    [features, points] = extractFeatures(grayImage, points);
...
% The following estimateGeometricTransform is for the cinder wall images
%     tforms(n) = estimateGeometricTransform(matchedPoints, matchedPointsPrev,...
%         'projective', 'Confidence', 90, 'MaxNumTrials', 1000, 'MaxDistance', 2.3);
